...
export class SessionService {
  ...
  async continueSession(
    ...
    if (session.currentSection) {
      ...
      currentSectionInfo = {
        ...
        description: (session.currentSection).description ?? undefined,
    ...
    // 2. If this is a readiness-gated project type, score >= 95%
    const unansweredRequired = visibleQuestions.filter(
      (q) => q.isRequired && !responseMap.has(q.id),
    );
    const requiresReadinessGate = await this.isReadinessGatedSession(session);
    const meetsReadinessGate =
      !requiresReadinessGate || (readinessScore ?? 0) >= READINESS_SCORE_THRESHOLD;
    const canComplete = unansweredRequired.length === 0 && answeredCount > 0 && meetsReadinessGate;
  ...
  private async isReadinessGatedSession(session: {
