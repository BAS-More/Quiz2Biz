...
export function IdeaCapturePage() {
  ...
  const handleConfirmAndStart = useCallback(async () => {
    ...
    try {
      ...
      const selectedType = allTypes.find((t) => t.slug === selectedProjectTypeSlug);

      if (selectedType && selectedProjectTypeSlug !== ideaResponse.analysis.recommendedProjectType.slug) {
        // Need to confirm the alternative project type via API
        // For now, the projectTypeId was already set during analysis
      }
