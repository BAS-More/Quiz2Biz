...
const styles = {
  ...
  navItem:
    'flex items-center gap-3 px-5 py-3 text-white/70 no-underline text-sm font-medium cursor-pointer transition-all duration-200 border-none bg-transparent w-full text-left',
  ...
  suggestionButton:
    'px-4 py-2 bg-brand-500 text-white border-none rounded-md text-xs font-medium cursor-pointer mr-2',
  suggestionDismiss:
    'px-3 py-2 bg-transparent text-white/60 border border-white/20 rounded-md text-xs cursor-pointer',
  quickAccessItem:
    'flex items-center gap-2 px-3 py-2 bg-white/5 rounded-md mb-1 cursor-pointer text-[13px]',
  removeButton:
    'ml-auto p-1 bg-transparent border-none text-white/40 cursor-pointer text-xs opacity-0 transition-opacity duration-200',
...
export const NextActionSuggestion: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div>
        <p style={{ fontSize: '12px', color: 'var(--color-surface-500)', margin: 0 }}>
          Suggested next
        </p>
        <p
          style={{ fontSize: '14px', fontWeight: 500, color: 'var(--color-brand-800)', margin: 0 }}
        >
          {predictedItem.label}
        </p>
      </div>
