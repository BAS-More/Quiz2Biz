...
const styles = {
  presenceBar: 'flex items-center gap-2 px-4 py-2 bg-surface-50 rounded-lg border border-surface-200',
  presenceLabel: 'text-[13px] text-surface-500 mr-2',
  avatarStack: 'flex items-center',
  avatar: 'w-8 h-8 rounded-full border-2 border-surface-50 -ml-2 flex items-center justify-center text-xs font-semibold text-white cursor-pointer relative',
  avatarFirst: 'ml-0',
  onlineIndicator: 'absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-surface-50',
  ...
  cursorPointer: 'w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[10px]',
  cursorLabel: 'mt-0.5 px-1.5 py-0.5 rounded text-[11px] font-medium text-white whitespace-nowrap',
  lockBadge: 'inline-flex items-center gap-1 px-2 py-1 text-xs rounded bg-warning-50 text-warning-600',
  ...
  conflictVersionContent: 'text-sm text-surface-800 leading-normal bg-surface-50 p-3 rounded-md font-mono min-h-[80px] whitespace-pre-wrap',
  conflictActions: 'flex gap-3 justify-end',
  conflictButton: 'px-5 py-2.5 text-sm font-medium rounded-lg border-none cursor-pointer transition-colors',
  ...
  mergeTextarea: 'w-full min-h-[100px] p-3 text-sm border border-surface-200 rounded-lg resize-y mb-4 font-mono',
  typingIndicator: 'flex items-center gap-2 px-3 py-2 text-[13px] text-surface-500 bg-surface-50 rounded-lg mt-2',
        ...
        {visibleUsers.map((user, index) => (
          ...
          >
            ...
            {hoveredUser === user.userId && (
              <div
                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-surface-800 text-white text-xs rounded whitespace-nowrap"
              >
                {user.userName}
              </div>
            )}
        ...
        <div className={styles.conflictBody}>
          <div className={styles.conflictVersions}>
            <div className={styles.conflictVersion}>
              <div className={styles.conflictVersionHeader}>Your Version ({conflict.localUser})</div>
              <div className={styles.conflictVersionContent}>{conflict.localValue}</div>
            </div>
            <div className={styles.conflictVersion}>
              <div className={styles.conflictVersionHeader}>Their Version ({conflict.remoteUser})</div>
