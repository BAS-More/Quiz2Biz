...
exports.heatmapCommand = new commander_1.Command('heatmap')
    ...
    var config, targetSessionId, spinner, client, heatmapData, output, extension, outputPath, error_1;
        ...
        switch (_a.label) {
            ...
            case 2:
                ...
                if (options.stdout) {
                    ...
                    outputPath = options.output || "quiz2biz-heatmap-".concat(targetSessionId.substring(0, 8)).concat(extension);
                    fs.writeFileSync(outputPath, output);
                    console.log(chalk_1.default.green("\u2705 Heatmap exported to: ".concat(path.resolve(outputPath))));
                }
                return [3 /*break*/, 4];
            case 3:
                error_1 = _a.sent();
...
function formatMarkdown(data) {
    ...
    data.dimensions.forEach(function (dim) {
        var status = dim.score >= 0.95 ? '🟢' : dim.score >= 0.7 ? '🟡' : dim.score >= 0.4 ? '🟠' : '🔴';
        lines.push("| ".concat(dim.name, " | ").concat((dim.score * 100).toFixed(0), "% | ").concat(dim.questionsAnswered, "/").concat(dim.totalQuestions, " | ").concat(status, " |"));
    });
    ...
    data.dimensions.forEach(function (dim) {
        ...
        dim.questions.forEach(function (q) {
            ...
            var status = residual <= 0.05 ? '🟢' : residual <= 0.15 ? '🟡' : residual <= 0.3 ? '🟠' : '🔴';
            var questionText = q.text.length > 50 ? q.text.substring(0, 47) + '...' : q.text;
            lines.push("| ".concat(questionText, " | ").concat((q.severity * 100).toFixed(0), "% | ").concat((q.coverage * 100).toFixed(0), "% | ").concat(status, " |"));
        });
        lines.push('');
    });
    return lines.join('\n');
...
function formatCSV(data) {
    ...
    data.dimensions.forEach(function (dim) {
        dim.questions.forEach(function (q) {
            ...
            rows.push("".concat(dim.name, ",").concat(q.id, ",").concat(escapedText, ",").concat(q.severity.toFixed(2), ",").concat(q.coverage.toFixed(2), ",").concat(residual.toFixed(2), ",").concat(status));
        });
    });
    return rows.join('\n');
}
