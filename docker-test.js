...
function runLocalDockerTests() {
  ...
  const dockerVersion = runCommand('docker version --format "{{.Client.Version}}"', {
    timeout: 10000,
  });
  ...
  if (networks.success) {
    ...
    log(
      `Quest network: ${hasNetwork ? 'exists' : 'will be created on start'}`,
      hasNetwork ? 'success' : 'info',
    );
  ...
  if (containers.success) {
    ...
    recordTest(
      ...
      'Running Containers',
      true,
      `${running.length}/${CONFIG.local.services.length}`,
    );
  } else {
...
function runQuestTests() {
  ...
  if (existingPaths.length === requiredPaths.length) {
    ...
    log(
      `Missing files: ${requiredPaths.filter((p) => !existingPaths.includes(p)).join(', ')}`,
      'warn',
    );
  ...
  const healthCheck = runCommand(
    `curl -s -o /dev/null -w "%{http_code}" http://localhost:${CONFIG.local.ports.api}/health`,
    { timeout: 5000 },
  );
  ...
  const pgTest = runCommand(`docker exec questionnaire-postgres pg_isready -U postgres`, {
    timeout: 5000,
  });
  if (pgTest.success) {
...
function runCloudTests() {
  ...
  if (existingTf.length === tfFiles.length) {
    ...
    log(
      `Missing Terraform files: ${tfFiles.filter((f) => !existingTf.includes(f)).join(', ')}`,
      'warn',
    );
  ...
  recordTest(
    ...
    'Docker Hub',
    true,
    `hub.docker.com/repositories/${CONFIG.project.dockerHub}`,
  );

  // Test 7: ACR Configuration
  log('Checking Azure Container Registry settings...', 'step');
  if (azAccount.success) {
    const acrCheck = runCommand(`az acr show --name ${CONFIG.azure.acr} --output tsv 2>&1`, {
      timeout: 15000,
    });
    if (acrCheck.success) {
  ...
  if (azAccount.success) {
    const rgCheck = runCommand(
      `az group show --name ${CONFIG.azure.resourceGroup} --output tsv 2>&1`,
      { timeout: 15000 },
    );
    if (rgCheck.success) {
...
function printSummary() {
  ...
  categories.forEach((cat) => {
    ...
    console.log(
      `${status}${cat.name}:${colors.reset} ${r.passed} passed, ${r.failed} failed, ${r.skipped} skipped`,
    );
  });
