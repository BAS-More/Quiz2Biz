...
# Prisma binary path (avoid npx downloading latest version)
...
docker compose exec -T api $PRISMA_BIN migrate deploy 2>/dev/null && {
    echo -e "${GREEN}  [OK] Migrations applied${NC}"
} || {
    echo -e "${YELLOW}  [INFO] Retrying migrations...${NC}"
    sleep 10
    docker compose exec -T api $PRISMA_BIN migrate deploy || echo -e "${YELLOW}  [WARN] Migration may need manual review${NC}"
}
...
docker compose exec -T api $PRISMA_BIN db seed 2>/dev/null && {
...
echo -e "  DB Studio:     docker compose exec api $PRISMA_BIN studio"
